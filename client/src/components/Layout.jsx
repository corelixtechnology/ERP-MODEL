...
function Layout() {
  ...
  const getNavItems = (role) => {
    ...
    const roleSpecificItems = {
      ...
      faculty: [
        { text: 'My Courses', icon: <BookIcon />, path: '/faculty/courses' },
        { text: 'Mark Attendance', icon: <AttendanceIcon />, path: '/faculty/attendance' },
        { text: 'Enter Grades', icon: <GradeIcon />, path: '/faculty/grades' },
      ],
      hod: [
        { text: 'My Courses', icon: <BookIcon />, path: '/faculty/courses' },
        { text: 'Mark Attendance', icon: <AttendanceIcon />, path: '/faculty/attendance' },
      ...
      student: [
        { text: 'My Attendance', icon: <AttendanceIcon />, path: '/student/attendance' },
        { text: 'My Grades / Results', icon: <GradeIcon />, path: '/student/results' },
        { text: 'Fee Payments', icon: <PaymentIcon />, path: '/student/fees' },
        { text: 'Library Books', icon: <LibraryIcon />, path: '/student/library' },
      ],
      accounts: [
        { text: 'Transactions', icon: <PaymentIcon />, path: '/accounts/transactions' },
        { text: 'Fee Structure', icon: <AccountsIcon />, path: '/accounts/fees' },
        { text: 'Financial Reports', icon: <LibraryIcon />, path: '/accounts/reports' },
      ],
      accountant: [
        { text: 'Transactions', icon: <PaymentIcon />, path: '/accounts/transactions' },
        { text: 'Fee Structure', icon: <AccountsIcon />, path: '/accounts/fees' },
        ...
        <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 2, sm: 3 }, minHeight: 64 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" noWrap component="div" sx={{ fontWeight: 800, color: '#0f172a', fontSize: '1.1rem' }}>
              {menuItems.find((item) => item.path === location.pathname)?.text || 'ERP Dashboard'}
            </Typography>
          </Box>

          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <IconButton size="small" sx={{ color: '#64748b', '&:hover': { color: '#0ea5e9', bgcolor: 'rgba(14, 165, 233, 0.08)' } }}>
              <Badge badgeContent={2} color="primary" sx={{ '& .MuiBadge-badge': { bgcolor: '#0ea5e9' } }}>
            ...
            {user && (
              <Chip
                label={user.role ? user.role.toUpperCase().replace('_', ' ') : 'USER'}
            ...
            <Tooltip title="Account settings">
              <IconButton onClick={handleMenuOpen} size="small" sx={{ p: 0.5 }}>
                <Avatar
                  alt={user?.name}
                  src={user?.profilePic}
                  sx={{ width: 36, height: 36, bgcolor: '#0ea5e9', fontSize: '0.85rem', fontWeight: 700 }}
                >
                  {user?.name ? user.name.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase() : 'U'}
                </Avatar>
              </IconButton>
